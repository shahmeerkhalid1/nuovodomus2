...
const ProjectsSliderFour = ({ text_muted, bg_muted }) => {
    ...
    return (
        <section className='pt-10 md:pt-16 lg:pt-20'>
            ...
            <div className='container-fluid px-4 sm:px-6 lg:px-8 relative pt-10 sm:pt-14 lg:pt-20'>
                ...
                >
                    {
                        projectsData.map(({ id, area, client, project_img, project_type, project_year }) => {
                            return (
                                <SwiperSlide key={id}>
                                    <div 
                                        className='bg-cover bg-no-repeat bg-center w-full h-[300px] sm:h-[400px] md:h-[500px] 2xl:h-[570px]' 
                ...
                <div className='flex flex-col xl:flex-row items-stretch justify-center xl:justify-start gap-4 lg:gap-6'>
                    <div className={cn(`project-pagination w-full max-w-full  2xl:max-w-[800px] bg-primary px-4 sm:px-6 lg:px-4 py-3 relative z-[1] mt-[-20px] sm:mt-[-30px] ${bg_muted}`)}></div>
                    <div className='flex flex-col justify-between w-full xl:w-auto'>
                        <div className='flex flex-col sm:flex-row justify-center xl:justify-end gap-4 sm:gap-6 lg:gap-6 xl:gap-10 my-4 px-4 sm:px-0'>
                            <h6 className='flex flex-col text-lg sm:text-xl lg:text-2xl font-bold leading-tight text-primary-foreground whitespace-nowrap'>
                                Clients:
                                <span className='text-sm sm:text-base lg:text-lg font-normal whitespace-nowrap'>{projectsData[activeIndex].client}</span>
                            </h6>
                            <h6 className='flex flex-col text-lg sm:text-xl lg:text-2xl font-bold leading-tight text-primary-foreground'>
                                Area:
                                <span className='text-sm sm:text-base lg:text-lg font-normal whitespace-nowrap'>{projectsData[activeIndex].area}</span>
                            </h6>
                            <h6 className='flex flex-col text-lg sm:text-xl lg:text-2xl font-bold leading-tight text-primary kaikkk-foreground whitespace-nowrap'>
                                Project year:
                                <span className='text-sm sm:text-base lg:text-lg font-normal whitespace-nowrap'>{projectsData[activeIndex].project_year}</span>
                            </h6>
                            <h6 className='flex flex-col text-lg sm:text-xl lg:text-2xl font-bold leading-tight text-primary-foreground whitespace-nowrap'>
                                Project type:
                                <span className='text-sm sm:text-base lg:text-lg font-normal whitespace-nowrap'>{projectsData[activeIndex].project_type}</span>
                            </h6>
                        </div>
                        <div className='flex items-start justify-center xl:justify-start gap-2 sm:gap-3 lg:gap-4 px-4 sm:px-0'>
                            <div onClick={() => swiperRef.current?.slidePrev()}>
                                <ButtonFill className={"rotate-180 h-12 w-12 sm:h-16 sm:w-16 lg:h-[50px] lg:w-[75px] px-2 sm:px-3 after:bg-secondary border-secondary hover:border-primary hover:bg-primary hover:text-secondary text-primary-foreground"}>
                                    <RightArrow width={"24"} height={"16"} className="sm:w-[30px] sm:h-[20px] lg:w-[35px] lg:h-[22px]" />
                                </ButtonFill>
                            </div>
                            <div onClick={() => swiperRef.current?.slideNext()}>
                                <ButtonFill className={cn(`h-12 sm:h-16 lg:h-[50px] px-4 sm:px-6 lg:px-10 text-sm sm:text-base after:left-0 after:${bg_muted} hover:border-primary`)}>
                                    Next Project 
